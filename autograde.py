...
#    df = df.loc[(df["function"] == "bench_solution")].copy().reset_index()
    ...
    return scores[['label',
                   'size', 'power',
                   'target_speedup', baseFOM, FOM, 'speedup', 'weight','bench_score', 'score']]
...
def compute_all_scores(dir=None):
    ...
    if canary["ET"].iloc[0] >  0.68:
        raise CanaryException(f"your canary ran too slowly ({canary['ET'].iloc[0]}).  Rerun your code")

    bench["label"] = bench["function"] + "-"
    bench["label"] = bench["label"] + list(map(str,bench["size"])) + " " + list(map(str,bench["power"]))

    params = [
        # baseline ET  Speedup  weight
        (0.68,            1,   0),
        (3.2,            round(114 * 0.9, 2),  1/3.0),
        (7.7,            round(20 * 0.9,  2),  1/3.0),   # 
        (3.2,            round(9.4 * 0.9, 2),  1/3.0)  # 1.32
    ]
