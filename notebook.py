...
os.environ['CSE142L_LAB'] = 'parallel'
...
def build_reps(src, asm, obj, function, mtrace=None, stats=None, gmon=None, run=None, *argc, **kwargs):
    ...
    if function:
        try :
            ...
            print("=====  Couldn't Build CFG.  Sorry ========")
            cfg_counts = None
            cfg = None
    else:
        cfg_counts = None
...
def fiddle(fname, function=None, compile=True, name=None, code=None, opt=None, run=None, cmdline=None, perf_cmdline=None, analyze=True, **kwargs):
    ...
    if compile_together:
        ...
        r =  shell_cmd(f"make {base}.s C_OPTS='{opt}' fiddle.exe FIDDLE_OBJS={doto} {'GPROF=yes' if gprof else ''}", shell=True)
    else:
        #shell_cmd(f"rm -f build/{base}.s {so} build/{base}.o")
        r = shell_cmd(f"make {base}.s {so} C_OPTS='{opt}' fiddle.exe {'GPROF=yes' if gprof else ''}", shell=True)
    if not r:
        print("Build failed")
    ...
    for method in run:
        ...
        if compile_together:
            cmd = f"./fiddle.exe --function {' '.join(function)}"
        else:
            cmd = f"./fiddle.exe --lib ./{so} --function {' '.join(function)}"
        ...
        if method == "perf_count":
            ...
            cmd += f" {cmdline}"
            print(f"Executing: {cmd}")
            do_mtrace(trace=name.split("/")[-1], jupyter=True, tagged_only=tagged_only, cmd=cmd.split())
...
def login(username):
    ...
    if not re.match("[a-zA-Z0-9\-\.\_]+@[a-zA-Z0-9\-\.\_]+",username):
        return HTML("You need to enter your email above")
...
def do_render_code(file, lang="c++", show=None, line_numbers=True, trim_ends=False, demangle=None):
    ...
    if isinstance(show, str):
        if lang == "c++":
            ...
            raise Exception(f"Don't know how to find functions in {lang}")
...
def render_csv(file, columns = None, sort_by=None, average_by=None, skip=0, add_file=False):
...
def plotPEBar(file=None, what=None, df=None,columns=4, log=False, average=False, average_by=None, include_numbers=False, skip=0, height=1):
    if df is None:
        df = render_csv(file,average_by=average_by)
