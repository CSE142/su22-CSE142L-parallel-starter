...
struct tensor_t
{
	...
	*/
	...
	T* do_new(size_t size, bool init) {
		void * p = NULL;
		
		int r =  posix_memalign(&p, 2*1024*1024, size * sizeof(T));
		if (r == -1) { 
	...
	tensor_t( int _x, int _y=1, int _z=1, int _b=1, bool zero=true,  T* memory=NULL ) :  size(_x, _y, _z, _b), delete_memory(true) {
		if (memory) {
			data = memory;
			delete_memory=false;
		} else {
			if (zero) {
				data = do_new(size.x * size.y * size.z * size.b, true);
			} else {
				data = do_new(size.x * size.y * size.z * size.b, false);
			}
	...
	tensor_t<T> & operator=(const tensor_t& other )
	{
		if (&other != this) {
			do_delete(data);
			size = other.size;
			data =do_new(other.size.x * other.size.y * other.size.z * other.size.b, false);
			memcpy(
				this->data,
				other.data,
				calculate_data_size()
