...
int main(int argc, char *argv[])
{
	...
	archlab_add_multi_option<std::vector<unsigned long int> >("arg2",
					      arg2s,
					      default_arg2s,
					      "1",
					      "Arg2.  Pass multiple values to run with multiple arg2s.");
	std::vector<unsigned long int> arg3s;
	std::vector<unsigned long int> default_arg3s;
	default_arg3s.push_back(1);
	archlab_add_multi_option<std::vector<unsigned long int> >("arg3",
					      arg3s,
					      default_arg3s,
					      "1",
					      "Arg3.  Pass multiple values to run with multiple arg3s.");
	...
	archlab_add_multi_option<std::vector<int32_t> >("tile-size",
					      tile_sizes,
					      default_tile_sizes,
					      "64",
					      "Tile_Size.  Pass multiple values to run with multiple tile_sizes.");
									...
									for(auto & size3: sizes3 ) {
										...
										for(auto & function : functions) {
											...
											for(uint r = 0; r < reps; r++) {
												...
												theDataCollector->register_tag("arg2",arg2);
												theDataCollector->register_tag("arg3",arg3);
												theDataCollector->register_tag("thread",thread);
												theDataCollector->register_tag("tile_size",tile_size);
